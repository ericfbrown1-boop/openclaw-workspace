#!/usr/bin/env python3
import imaplib
import email
from email.header import decode_header
import os
from datetime import datetime

# Gmail credentials
EMAIL = "ericfbrown1@gmail.com"
PASSWORD = "jplmfcfecipqwkgi"
IMAP_SERVER = "imap.gmail.com"

def connect_gmail():
    """Connect to Gmail via IMAP"""
    print(f"Connecting to {IMAP_SERVER}...")
    mail = imaplib.IMAP4_SSL(IMAP_SERVER)
    mail.login(EMAIL, PASSWORD)
    print("✓ Connected successfully")
    return mail

def search_sent_folder(mail):
    """Search for emails in Sent folder from today"""
    print("\nSearching Gmail folders...")
    
    # Try different possible sent folder names
    sent_folders = ['[Gmail]/Sent Mail', 'Sent', 'SENT', '[Gmail]/Sent']
    
    for folder in sent_folders:
        try:
            status, messages = mail.select(f'"{folder}"')
            if status == 'OK':
                print(f"✓ Found sent folder: {folder}")
                
                # Search for emails from today
                today = datetime.now().strftime("%d-%b-%Y")
                print(f"Searching for emails from: {today}")
                
                # Search for emails from today
                status, msg_ids = mail.search(None, f'SINCE {today}')
                
                if status == 'OK':
                    msg_id_list = msg_ids[0].split()
                    print(f"Found {len(msg_id_list)} email(s) from today")
                    return mail, msg_id_list, folder
        except Exception as e:
            print(f"Could not access {folder}: {e}")
            continue
    
    return mail, [], None

def list_all_emails(mail, msg_ids):
    """List all emails with their attachments"""
    print("\n" + "="*80)
    print("ALL SENT EMAILS FROM TODAY:")
    print("="*80)
    
    all_attachments = []
    
    for idx, msg_id in enumerate(reversed(msg_ids), 1):  # Start with most recent
        status, msg_data = mail.fetch(msg_id, '(RFC822)')
        
        if status != 'OK':
            continue
            
        # Parse email
        msg = email.message_from_bytes(msg_data[0][1])
        
        # Get subject
        subject = msg.get('Subject', '')
        if subject:
            subject_decoded = decode_header(subject)[0]
            if isinstance(subject_decoded[0], bytes):
                subject = subject_decoded[0].decode()
            else:
                subject = subject_decoded[0]
        
        # Get date
        date_str = msg.get('Date', '')
        
        print(f"\n[{idx}] Subject: {subject}")
        print(f"    Date: {date_str}")
        
        # Check for attachments
        has_attachment = False
        if msg.is_multipart():
            for part in msg.walk():
                filename = part.get_filename()
                
                if filename:
                    has_attachment = True
                    # Decode filename if needed
                    decoded = decode_header(filename)
                    if isinstance(decoded[0][0], bytes):
                        filename = decoded[0][0].decode(decoded[0][1] or 'utf-8')
                    else:
                        filename = decoded[0][0]
                    
                    print(f"    📎 Attachment: {filename}")
                    all_attachments.append({
                        'msg_id': msg_id,
                        'subject': subject,
                        'filename': filename,
                        'part': part,
                        'index': idx
                    })
        
        if not has_attachment:
            print(f"    (No attachments)")
    
    return all_attachments

def download_specific_attachment(mail, attachments, search_terms=['NARR', 'Feb', '2026']):
    """Download attachment matching search terms"""
    print("\n" + "="*80)
    print(f"SEARCHING FOR ATTACHMENT WITH: {', '.join(search_terms)}")
    print("="*80)
    
    for att in attachments:
        filename = att['filename']
        match_score = sum(1 for term in search_terms if term.upper() in filename.upper())
        
        if match_score >= 2:  # At least 2 terms match
            print(f"\n✓ FOUND MATCH: {filename}")
            print(f"  From email: {att['subject']}")
            
            # Download the attachment
            filepath = os.path.join(os.getcwd(), filename)
            with open(filepath, 'wb') as f:
                f.write(att['part'].get_payload(decode=True))
            
            print(f"✓ Downloaded to: {filepath}")
            return filepath, att['subject']
    
    # If no match, try just 'NARR'
    print("\nNo exact match found. Trying broader search for 'NARR'...")
    for att in attachments:
        if 'NARR' in att['filename'].upper():
            print(f"\n✓ FOUND: {att['filename']}")
            print(f"  From email: {att['subject']}")
            
            filepath = os.path.join(os.getcwd(), att['filename'])
            with open(filepath, 'wb') as f:
                f.write(att['part'].get_payload(decode=True))
            
            print(f"✓ Downloaded to: {filepath}")
            return filepath, att['subject']
    
    return None, None

def main():
    try:
        # Connect to Gmail
        mail = connect_gmail()
        
        # Search sent folder
        mail, msg_ids, folder = search_sent_folder(mail)
        
        if not msg_ids:
            print("\n❌ No emails found from today in Sent folder")
            mail.logout()
            return
        
        # List all emails
        attachments = list_all_emails(mail, msg_ids)
        
        if not attachments:
            print("\n❌ No attachments found in today's sent emails")
            mail.logout()
            return
        
        # Download NARR memo
        filepath, subject = download_specific_attachment(mail, attachments)
        
        if filepath:
            print(f"\n{'='*80}")
            print(f"✓ SUCCESS: Downloaded NARR Memo")
            print(f"File: {filepath}")
            print(f"Original email subject: {subject}")
            print(f"{'='*80}")
        else:
            print("\n❌ Could not find NARR Memo attachment")
            print("\nPlease check the list above and let me know which attachment to download.")
        
        # Logout
        mail.logout()
        print("\n✓ Disconnected from Gmail")
        
    except Exception as e:
        print(f"\n❌ Error: {e}")
        import traceback
        traceback.print_exc()

if __name__ == "__main__":
    main()
